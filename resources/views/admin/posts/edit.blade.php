            ...
            <div class="row g-4">
                <div class="col-md-8">
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" class="form-control form-control-lg rounded-3" value="{{ old('title', $post->title) }}" required>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">Short Excerpt / Summary</label>
                        <textarea name="excerpt" rows="3" class="form-control rounded-3">{{ old('excerpt', $post->excerpt) }}</textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-bold">Full Article Content (HTML allowed) <span class="text-danger">*</span></label>
                        <textarea name="content" rows="12" class="form-control rounded-3" required>{{ old('content', $post->content) }}</textarea>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="card bg-light border-0 rounded-4 p-3 mb-4">
                        ...
                        @if($post->image)
                            <div class="mb-3 text-center">
                                <img src="{{ Str::startsWith($post->image, ['http://', 'https://']) ? $post->image : asset($post->image) }}" class="rounded-3 img-fluid border" style="max-height: 150px; object-fit: cover;">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" name="image_url" class="form-control form-control-sm rounded-3" value="{{ old('image_url', Str::startsWith($post->image, ['http://', 'https://']) ? $post->image : '') }}">
                        </div>
                    </div>
                    ...
                    <div class="card bg-light border-0 rounded-4 p-3 mb-4">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="author" class="form-control rounded-3" value="{{ old('author', $post->author) }}" required>
                        </div>
