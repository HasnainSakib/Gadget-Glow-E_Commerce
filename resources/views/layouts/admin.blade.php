...
<head>
    ...
    <title>@yield('title', 'Admin Dashboard - Gadget & Glow')</title>
    <!-- Favicon -->
...
<body>
    ...
    <aside class="sidebar d-flex flex-column py-3">
        <div class="px-4 mb-3">
            <a href="{{ route('admin.dashboard') }}" class="text-dark text-decoration-none d-flex align-items-center gap-2">
                <img src="{{ $faviconUrl }}" style="width: 28px; height: 28px; object-fit: contain;">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item">
                ...
                @endphp
                <a href="{{ route('admin.orders.bin') }}" class="nav-link d-flex align-items-center justify-content-between {{ request()->routeIs('admin.orders.bin') ? 'active' : '' }}">
                    <span><i class="bi bi-trash3-fill text-danger me-2"></i> Order Bin (Trash)</span>
                    @if($trashedOrdersCount > 0)
                        <span class="badge bg-danger rounded-pill">{{ $trashedOrdersCount }}</span>
                    @endif
                </a>
            </li>
            <li class="nav-item">
                <a href="{{ route('admin.wishlists.index') }}" class="nav-link {{ request()->routeIs('admin.wishlists.*') ? 'active' : '' }}">
    ...
    <main class="main-content">
        ...
        <header class="admin-top-bar d-flex justify-content-between align-items-center mb-4">
            <div>
                <h3 class="fw-bold mb-0 text-dark">@yield('header_title', 'Dashboard Overview')</h3>
                    ...
                    <div class="dropdown-menu dropdown-menu-end border-0 shadow-lg p-0 mt-2 rounded-4" 
                        ...
                        <div class="p-3 border-bottom d-flex justify-content-between align-items-center bg-light rounded-top-4">
                            <div>
                                ...
                                <small class="text-muted">{{ $unreadNotificationsCount ?? 0 }} unread items</small>
                            </div>
                            @if(($unreadNotificationsCount ?? 0) > 0)
                        ...
                        <div class="list-group list-group-flush">
                            @forelse(($adminNotifications ?? []) as $notif)
                                <a href="{{ $notif->read_url }}" class="list-group-item list-group-item-action p-3 d-flex align-items-start gap-3 border-bottom text-decoration-none">
                                    <div class="rounded-circle p-2 d-flex align-items-center justify-content-center shrink-0 {{ $notif->icon_bg }}" style="width: 38px; height: 38px;">
                                        <i class="bi {{ $notif->icon }} fs-6"></i>
                                    </div>
                                    <div class="flex-grow-1 overflow-hidden">
                                        <div class="d-flex justify-content-between align-items-center mb-1">
                                            <strong class="text-dark small text-truncate d-block mb-0">{{ $notif->title }}</strong>
                                            <small class="text-muted" style="font-size: 0.7rem;">{{ $notif->created_at ? $notif->created_at->diffForHumans() : '' }}</small>
                                        </div>
                                        <p class="text-secondary small mb-0 text-truncate" style="font-size: 0.8rem;">{{ $notif->subtitle }}</p>
                                    </div>
                                </a>
                            @empty
                        ...
                        <div class="p-2 text-center bg-light border-top rounded-bottom-4">
                            <a href="{{ route('admin.orders.index') }}" class="small text-decoration-none text-primary fw-semibold">View All Orders</a>
                            <span class="text-muted mx-1">•</span>
                            <a href="{{ route('admin.contact.index') }}" class="small text-decoration-none text-primary fw-semibold">View Messages</a>
                        </div>
                    </div>
                </div>
