...
@section('title', $product->name . ' - Gadget & Glow')
...
<div class="container py-4">
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active fw-semibold text-dark" aria-current="page">{{ $product->name }}</li>
        </ol>
    </nav>
            ...
            <div class="row g-5 align-items-center">
                ...
                <div class="col-md-6 text-center">
                    <div class="position-relative overflow-hidden rounded-4 shadow-sm border bg-light p-2">
                        <img src="{{ $product->image }}" alt="{{ $product->name }}" class="img-fluid rounded-4" style="max-height: 420px; object-fit: cover; width: 100%;" onerror="this.onerror=null; this.src='data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'600\' height=\'600\' viewBox=\'0 0 600 600\'><rect width=\'100%\' height=\'100%\' fill=\'%23faf7f2\'/><text x=\'50%\' y=\'50%\' dominant-baseline=\'middle\' text-anchor=\'middle\' font-size=\'24\' fill=\'%2378716c\'>Product</text></svg>';">
                        <div class="position-absolute top-0 start-0 m-3">
                            <span class="badge-category {{ $product->category->slug == 'tech-gadgets' ? 'badge-gadget' : 'badge-beauty' }}">
                ...
                <div class="col-md-6">
                    <h2 class="fw-extrabold text-dark mb-3 brand-font">{{ $product->name }}</h2>

                    <div class="d-flex align-items-center gap-3 mb-3">
                        <span class="fs-2 fw-extrabold text-primary">৳{{ number_format($product->price, 0) }}</span>
                        @if($product->stock > 0)
                            <span class="badge bg-success-subtle text-success border border-success-subtle px-3 py-2 rounded-pill fw-semibold">
                                <i class="bi bi-check-circle-fill me-1"></i> In Stock ({{ $product->stock }} items)
                            </span>
                        @else
                    ...
                    <form action="{{ route('cart.add', $product->id) }}" method="POST" class="d-flex flex-column gap-3">
                        @csrf
                        <div class="d-flex align-items-center gap-3">
                            <label for="quantity" class="fw-semibold text-dark mb-0">Quantity:</label>
                            <input type="number" name="quantity" id="quantity" value="1" min="1" max="{{ $product->stock }}" class="form-control text-center rounded-3 fw-bold bg-white text-dark border" style="width: 90px;" {{ $product->stock <= 0 ? 'disabled' : '' }}>
                        </div>

                        <div class="d-flex flex-wrap gap-2 mt-2">
                            <button type="submit" class="btn btn-primary-custom btn-lg rounded-pill px-4 flex-grow-1 shadow-sm" {{ $product->stock <= 0 ? 'disabled' : '' }}>
                                <i class="bi bi-cart-plus-fill me-2"></i> Add to Cart
                            </button>
                            <button type="submit" formaction="{{ route('cart.buyNow', $product->id) }}" class="btn btn-success btn-lg rounded-pill px-4 flex-grow-1 shadow-sm fw-bold" style="background-color: #10b981; border-color: #10b981; color: white;" {{ $product->stock <= 0 ? 'disabled' : '' }}>
                                <i class="bi bi-lightning-charge-fill me-1"></i> Buy Now / সরাসরি ক্রয় করুন
                            </button>
                            <button type="button" class="btn btn-outline-danger btn-lg rounded-pill px-3 shadow-sm" data-bs-toggle="modal" data-bs-target="#wishlistModal{{ $product->id }}" title="Add to Wishlist">
            ...
            @foreach($relatedProducts as $rel)
                <div class="col-6 col-md-3">
                    <div class="card product-card border-0 shadow-sm rounded-4">
                        <img src="{{ $rel->image }}" class="card-img-top" alt="{{ $rel->name }}" onerror="this.onerror=null; this.src='data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'600\' height=\'600\' viewBox=\'0 0 600 600\'><rect width=\'100%\' height=\'100%\' fill=\'%23faf7f2\'/><text x=\'50%\' y=\'50%\' dominant-baseline=\'middle\' text-anchor=\'middle\' font-size=\'24\' fill=\'%2378716c\'>Product</text></svg>';">
                        <div class="card-body p-3">
                            <h6 class="fw-bold text-truncate mb-1"><a href="{{ route('shop.show', $rel->slug) }}" class="text-dark text-decoration-none hover-primary">{{ $rel->name }}</a></h6>
                            <div class="fw-bold text-primary">৳{{ number_format($rel->price, 0) }}</div>
                        </div>
                    </div>
                </div>
            @endforeach
