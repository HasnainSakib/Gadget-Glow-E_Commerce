...
@section('title', $category->name . ' Collection - Gadget & Glow')
@section('meta_description', 'Explore authentic ' . $category->name . ' products with fast nationwide delivery in Bangladesh.')
...
<div class="container-fluid px-lg-4 px-xl-5 py-4">
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb bg-white p-3 rounded-pill shadow-sm border mb-0 d-inline-flex align-items-center">
            <li class="breadcrumb-item"><a href="{{ route('shop.index') }}" class="text-decoration-none text-muted"><i class="bi bi-house-door me-1"></i> Home</a></li>
            <li class="breadcrumb-item"><a href="{{ route('shop.index') }}#products-catalog" class="text-decoration-none text-muted">Store</a></li>
            <li class="breadcrumb-item active text-primary fw-bold" aria-current="page">{{ $category->name }}</li>
        </ol>
    </nav>

    <!-- Category Header Hero Banner -->
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4" style="background: linear-gradient(135deg, {{ $category->slug == 'tech-gadgets' ? '#eff6ff 0%, #ffffff 50%, #f0fdf4 100%' : '#fdf2f8 0%, #ffffff 50%, #fff7ed 100%' }});">
        <div class="card-body p-4 p-md-5">
            <div class="row align-items-center g-4">
                <div class="col-lg-8">
                    <span class="badge {{ $category->slug == 'tech-gadgets' ? 'bg-primary-subtle text-primary border border-primary' : 'bg-danger-subtle text-danger border border-danger' }} px-3 py-2 rounded-pill fw-bold mb-3 d-inline-flex align-items-center gap-1.5" style="font-size: 0.85rem;">
                        <i class="bi {{ $category->slug == 'tech-gadgets' ? 'bi-laptop' : 'bi-flower1' }} fs-6"></i> {{ $category->name }} Collection
                    </span>
                    <h1 class="display-5 fw-extrabold text-dark mb-2">{{ $category->name }}</h1>
                    <p class="lead text-secondary opacity-90 mb-4" style="font-size: 1.05rem;">
                        {{ $category->description ?? 'Discover our curated range of 100% authentic products with fast delivery across Bangladesh.' }}
                    </p>
                    <div class="d-flex align-items-center gap-3">
                        <span class="badge bg-dark text-white rounded-pill px-3 py-2 fs-6 fw-semibold">
                            <i class="bi bi-box-seam me-1"></i> {{ $products->total() }} Products Available
                        </span>
                        <a href="{{ route('shop.index') }}" class="btn btn-outline-dark rounded-pill px-3 fw-bold btn-sm">
                            <i class="bi bi-arrow-left me-1"></i> Back to Home
                        </a>
                    </div>
                </div>
                <div class="col-lg-4 d-none d-lg-block text-center">
                    <img src="{{ $category->image }}" alt="{{ $category->name }}" class="img-fluid rounded-4 shadow-sm border" style="max-height: 220px; width: 100%; object-fit: cover;" onerror="this.onerror=null; this.src='data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'600\' height=\'600\' viewBox=\'0 0 600 600\'><rect width=\'100%\' height=\'100%\' fill=\'%23faf7f2\'/><text x=\'50%\' y=\'50%\' dominant-baseline=\'middle\' text-anchor=\'middle\' font-size=\'24\' fill=\'%2378716c\'>Category</text></svg>';">
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row align-items-center g-3">
            ...
            <div class="col-lg-6">
                <div class="d-flex flex-wrap gap-2">
                    <a href="{{ route('shop.index') }}" class="btn btn-sm btn-outline-dark rounded-pill px-3">
                        <i class="bi bi-grid me-1"></i> All Categories
                    </a>
                    @foreach($allCategories as $cat)
                        <a href="{{ route('shop.category', $cat->slug) }}" class="btn btn-sm {{ $cat->id == $category->id ? 'btn-primary-custom fw-bold shadow-sm' : 'btn-outline-dark' }} rounded-pill px-3">
            ...
            <div class="col-lg-6">
                <form action="{{ route('shop.category', $category->slug) }}" method="GET" class="d-flex flex-column flex-sm-row gap-2">
                    <div class="input-group flex-grow-1">
                        <span class="input-group-text bg-light border-end-0 rounded-start-pill ps-3">
                            <i class="bi bi-search text-muted"></i>
                        </span>
                        <input type="text" name="search" class="form-control bg-light border-start-0 ps-0" placeholder="Search in {{ $category->name }}..." value="{{ request('search') }}">
                    </div>
    ...
    @if(request('search'))
        <div class="alert alert-info alert-dismissible fade show rounded-4 border-0 shadow-sm d-flex align-items-center justify-content-between mb-4" role="alert">
            <div>
                ...
                Showing search results for <strong>"{{ request('search') }}"</strong> in <strong>{{ $category->name }}</strong>
            </div>
            <a href="{{ route('shop.category', $category->slug) }}" class="btn btn-sm btn-outline-dark rounded-pill fw-bold ms-3">Clear Search</a>
        </div>
    @endif
                    ...
                    <div class="card product-card h-100 border-0 shadow-sm rounded-4 bg-white">
                        <div class="position-relative overflow-hidden" style="height: 210px;">
                            ...
                            <div class="position-absolute top-0 end-0 m-2">
                                <button type="button" class="btn btn-sm btn-light rounded-circle shadow-sm p-0 d-flex align-items-center justify-content-center text-danger" style="width: 32px; height: 32px;" data-bs-toggle="modal" data-bs-target="#wishlistModalSub{{ $product->id }}" title="Add to Wishlist">
                        ...
                        <div class="modal fade" id="wishlistModalSub{{ $product->id }}" tabindex="-1" aria-hidden="true">
                        ...
                        <div class="card-body d-flex flex-column p-3">
                            <h6 class="card-title fw-bold text-dark mb-1.5 text-truncate" title="{{ $product->name }}">
                            ...
                            <div class="d-flex align-items-center justify-content-between mt-auto pt-2 border-top">
                                ...
                                <div class="d-flex align-items-center gap-1">
                                    <form action="{{ route('cart.add', $product->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        <button type="submit" class="btn btn-primary-custom rounded-circle p-2 d-flex align-items-center justify-content-center shadow-sm" style="width: 34px; height: 34px;" title="Add to Cart" {{ $product->stock <= 0 ? 'disabled' : '' }}>
    ...
    @else
        ...
        <div class="text-center py-5 bg-white rounded-4 shadow-sm border my-4">
            ...
            <p class="text-muted">There are currently no products available in <strong>{{ $category->name }}</strong> matching your criteria.</p>
            <a href="{{ route('shop.category', $category->slug) }}" class="btn btn-primary-custom rounded-pill px-4">View All {{ $category->name }} Products</a>
        </div>
    @endif
