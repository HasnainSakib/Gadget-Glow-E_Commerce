...
@section('title', 'Order Placed Successfully - Gadget & Glow')
            ...
            <div class="card border-0 shadow-lg rounded-4 p-4 p-md-5 bg-white">
                ...
                <div class="alert alert-light border rounded-4 p-3 text-start mb-4">
                    ...
                    <div class="d-flex justify-content-between border-bottom pb-2 mb-2">
                        ...
                        <span class="badge {{ in_array($order->payment_method, ['bkash', 'nagad', 'rocket']) ? 'bg-warning-subtle text-dark border border-warning' : 'bg-success-subtle text-success border border-success' }} px-3 py-1.5 rounded-pill fw-bold">
                    ...
                    <div class="d-flex justify-content-between border-bottom pb-2 mb-2">
                        ...
                        <span class="text-end" style="max-width: 250px;">{{ $order->shipping_address }}</span>
                    </div>

                    <div class="d-flex justify-content-between">
                        <span class="text-muted small">Total Amount:</span>
                        <strong class="fs-5 text-dark">৳{{ number_format($order->total_amount, 0) }}</strong>
                    </div>
                </div>
                ...
                <div class="list-group mb-4 text-start">
                    @foreach($order->items as $item)
                        <div class="list-group-item d-flex justify-content-between align-items-center border-0 bg-light rounded-3 mb-2">
                            <div class="d-flex align-items-center gap-3">
                                <img src="{{ $item->product->image }}" class="rounded-2" style="width: 45px; height: 45px; object-fit: cover;">
                                <div>
                                    <div class="fw-semibold small text-truncate" style="max-width: 220px;">{{ $item->product->name }}</div>
                ...
                <div class="d-flex gap-3 justify-content-center">
                    ...
                    <a href="{{ route('admin.orders.show', $order->id) }}" class="btn btn-outline-dark rounded-pill px-4">
