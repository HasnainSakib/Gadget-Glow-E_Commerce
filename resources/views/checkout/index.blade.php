...
@section('title', 'Checkout - Gadget & Glow')
    ...
    <div class="row g-4">
            ...
            <div class="card border-0 shadow-sm rounded-4 p-4 p-md-5 bg-white">
                ...
                <form action="{{ route('checkout.store') }}" method="POST" id="checkoutForm">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="customer_name" id="customer_name" class="form-control rounded-3 bg-white text-dark border @error('customer_name') is-invalid @enderror" placeholder="John Doe" value="{{ old('customer_name', session('customer_name')) }}" required>
                        @error('customer_name')
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="email" name="customer_email" id="customer_email" class="form-control rounded-3 bg-white text-dark border @error('customer_email') is-invalid @enderror" placeholder="john@example.com" value="{{ old('customer_email', session('customer_email')) }}" required>
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="tel" name="customer_phone" id="customer_phone" minlength="11" maxlength="11" pattern="01[3-9][0-9]{8}" class="form-control rounded-3 bg-white text-dark border @error('customer_phone') is-invalid @enderror" placeholder="01712345678" value="{{ old('customer_phone', session('customer_phone')) }}" title="Please enter a valid 11-digit Bangladeshi mobile number starting with 01" required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="shipping_address" id="shipping_address" rows="3" class="form-control rounded-3 bg-white text-dark border @error('shipping_address') is-invalid @enderror" placeholder="House/Flat No, Road Name, Area, District" required>{{ old('shipping_address', session('shipping_address')) }}</textarea>
                    ...
                    <div class="d-flex flex-column gap-3 mb-4">
                        ...
                        <div class="card border rounded-4 p-3 payment-option-card" id="card_cod" style="transition: all 0.25s ease; cursor: pointer;">
                            <div class="form-check d-flex align-items-center gap-3 mb-0">
                                <input class="form-check-input mt-0 fs-5" type="radio" name="payment_method" id="pm_cod" value="cod" {{ old('payment_method', 'cod') == 'cod' ? 'checked' : '' }} onchange="togglePaymentInstructions()">
                        ...
                        <div class="card border rounded-4 p-3 payment-option-card" id="card_bkash" style="transition: all 0.25s ease; cursor: pointer;">
                            <div class="form-check d-flex align-items-center gap-3 mb-0">
                                <input class="form-check-input mt-0 fs-5" type="radio" name="payment_method" id="pm_bkash" value="bkash" {{ old('payment_method') == 'bkash' ? 'checked' : '' }} onchange="togglePaymentInstructions()">
                                <label class="form-check-label w-100 cursor-pointer" for="pm_bkash">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="d-flex align-items-center justify-content-center p-1 bg-white rounded-3 shadow-sm border border-pink overflow-hidden" style="width: 54px; height: 54px;">
                                            <img src="{{ $bkashLogoUrl }}" alt="bKash Logo" class="rounded-2" style="max-width: 100%; max-height: 100%; object-fit: contain;">
                                        </div>
                                    </div>
                                </label>
                            </div>
                        ...
                        <div class="card border rounded-4 p-3 payment-option-card" id="card_nagad" style="transition: all 0.25s ease; cursor: pointer;">
                            <div class="form-check d-flex align-items-center gap-3 mb-0">
                                <input class="form-check-input mt-0 fs-5" type="radio" name="payment_method" id="pm_nagad" value="nagad" {{ old('payment_method') == 'nagad' ? 'checked' : '' }} onchange="togglePaymentInstructions()">
                                <label class="form-check-label w-100 cursor-pointer" for="pm_nagad">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="d-flex align-items-center justify-content-center p-1 bg-white rounded-3 shadow-sm border border-warning overflow-hidden" style="width: 54px; height: 54px;">
                                            <img src="{{ $nagadLogoUrl }}" alt="Nagad Logo" class="rounded-2" style="max-width: 100%; max-height: 100%; object-fit: contain;">
                                        </div>
                                    </div>
                                </label>
                            </div>
                        ...
                        <div class="card border rounded-4 p-3 payment-option-card" id="card_rocket" style="transition: all 0.25s ease; cursor: pointer;">
                            <div class="form-check d-flex align-items-center gap-3 mb-0">
                                <input class="form-check-input mt-0 fs-5" type="radio" name="payment_method" id="pm_rocket" value="rocket" {{ old('payment_method') == 'rocket' ? 'checked' : '' }} onchange="togglePaymentInstructions()">
                                <label class="form-check-label w-100 cursor-pointer" for="pm_rocket">
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="d-flex align-items-center justify-content-center p-1 bg-white rounded-3 shadow-sm border border-purple overflow-hidden" style="width: 54px; height: 54px;">
                                            <img src="{{ $rocketLogoUrl }}" alt="Rocket Logo" class="rounded-2" style="max-width: 100%; max-height: 100%; object-fit: contain;">
                                        </div>
                                    </div>
                                </label>
                            </div>
        ...
        <div class="col-lg-5">
            <div class="card border-0 shadow-sm rounded-4 p-4 bg-white">
                <h5 class="fw-bold text-dark mb-3 border-bottom pb-2">Your Order ({{ count($cart) }} Items)</h5>

                <div class="mb-3" style="max-height: 300px; overflow-y: auto;">
                    @foreach($cart as $item)
                        <div class="d-flex align-items-center gap-3 py-2 border-bottom">
                            <img src="{{ $item['image'] }}" alt="{{ $item['name'] }}" class="rounded-3 border" style="width: 50px; height: 50px; object-fit: cover;" onerror="this.onerror=null; this.src='data:image/svg+xml;utf8,<svg xmlns=\'http://www.w3.org/2000/svg\' width=\'100\' height=\'100\'><rect width=\'100%\' height=\'100%\' fill=\'%23faf7f2\'/><text x=\'50%\' y=\'50%\' dominant-baseline=\'middle\' text-anchor=\'middle\' font-size=\'14\' fill=\'%2378716c\'>Item</text></svg>';">
                            <div class="flex-grow-1">
                                <h6 class="mb-0 text-truncate text-dark" style="max-width: 200px;">{{ $item['name'] }}</h6>
                                <small class="text-muted">Qty: {{ $item['quantity'] }} × ৳{{ number_format($item['price'], 0) }}</small>
                            </div>
